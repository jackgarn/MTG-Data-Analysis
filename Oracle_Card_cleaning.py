...
def export_df(data):
       data.to_excel("Oracle_Cards_Sheet.xlsx",index=False)
       data.to_csv("Oracle_Cards_CSV.csv",index=False)


# Gets Oracle cards from Scryfall and puts them in a pandas dataframe
response = requests.get('https://data.scryfall.io/oracle-cards/oracle-cards-20230126100206.json')
...
# Re-cast power, toughness, price as numerical values rather than strings
